...
// ============================================================================
// Testbench : tb_rv32i_store_axi_window_padded
...
module tb_rv32i_store_axi_window_padded;
    ...
    task preload_store_window_program;
        begin
            ...
            put_imem(  4, 32'h00000013); // padding NOP
            put_imem(  5, 32'h00000013); // padding NOP
            put_imem(  6, 32'h00100113); // addi x2,x0,1 ; store value
            put_imem(  7, 32'h00000013); // padding NOP
            put_imem(  8, 32'h00000013); // padding NOP
            put_imem(  9, 32'h1820A823); // sw x2,0x190(x1) ; result window
            put_imem( 10, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 11, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 12, 32'h1820AA23); // sw x2,0x194(x1) ; result window
            put_imem( 13, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 14, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 15, 32'h1820AC23); // sw x2,0x198(x1) ; result window
            put_imem( 16, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 17, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 18, 32'h1820AE23); // sw x2,0x19c(x1) ; result window
            put_imem( 19, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 20, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 21, 32'h1A20A023); // sw x2,0x1a0(x1) ; result window
            put_imem( 22, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 23, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 24, 32'h1A20A223); // sw x2,0x1a4(x1) ; result window
            put_imem( 25, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 26, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 27, 32'h1A20A423); // sw x2,0x1a8(x1) ; result window
            put_imem( 28, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 29, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 30, 32'h1A20A623); // sw x2,0x1ac(x1) ; result window
            put_imem( 31, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 32, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 33, 32'h1A20A823); // sw x2,0x1b0(x1) ; result window
            put_imem( 34, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 35, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 36, 32'h1A20AA23); // sw x2,0x1b4(x1) ; result window
            put_imem( 37, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 38, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 39, 32'h1A20AC23); // sw x2,0x1b8(x1) ; result window
            put_imem( 40, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 41, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 42, 32'h1A20AE23); // sw x2,0x1bc(x1) ; result window
            put_imem( 43, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 44, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 45, 32'h1C20A023); // sw x2,0x1c0(x1) ; result window
            put_imem( 46, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 47, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 48, 32'h1C20A223); // sw x2,0x1c4(x1) ; result window
            put_imem( 49, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 50, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 51, 32'h1C20A423); // sw x2,0x1c8(x1) ; result window
            put_imem( 52, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 53, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 54, 32'h1C20A623); // sw x2,0x1cc(x1) ; result window
            put_imem( 55, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 56, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 57, 32'h1C20A823); // sw x2,0x1d0(x1) ; result window
            put_imem( 58, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 59, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 60, 32'h1C20AA23); // sw x2,0x1d4(x1) ; result window
            put_imem( 61, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 62, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 63, 32'h1C20AC23); // sw x2,0x1d8(x1) ; result window
            put_imem( 64, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 65, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 66, 32'h1C20AE23); // sw x2,0x1dc(x1) ; result window
            put_imem( 67, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 68, 32'h00000013); // padding NOP after store for sync-fetch core
            put_imem( 69, 32'h100011B7); // lui x3,0x10001 ; GPIO base
            put_imem( 70, 32'h00000013); // padding NOP
            put_imem( 71, 32'h00000013); // padding NOP
            put_imem( 72, 32'hC0FFF237); // lui x4,0xC0FFF
            put_imem( 73, 32'h00000013); // padding NOP
            put_imem( 74, 32'h00000013); // padding NOP
            put_imem( 75, 32'hE0020213); // addi x4,x4,-512 ; x4=C0FFEE00
            put_imem( 76, 32'h00000013); // padding NOP
            put_imem( 77, 32'h00000013); // padding NOP
            put_imem( 78, 32'h0041A623); // sw x4,12(x3) ; DEBUG_OUT=C0FFEE00
            put_imem( 79, 32'h00000013); // padding NOP
            put_imem( 80, 32'h00000013); // padding NOP
            put_imem( 81, 32'h0000006F); // jal x0,0 ; loop
            $display("[TB] Store/AXI padded window program loaded. Instruction count=%0d", 82);
            $display("[TB] Store window offsets: 0x190..0x1DC, expected value=1");
            $display("[TB] NOTE: This padded version checks AXI/data-store correctness without dense-fetch skipping.");
        end
    endtask
    ...
    always @(posedge clk_pad) begin
        if (rst_n_pad) begin
            ...
            if ((pc_debug_pad >= 32'h00010020 && pc_debug_pad <= 32'h00010120) ||
